...
class AuthService {
  ...
  async register(credentials: RegisterCredentials): Promise<{ user: AuthUser; token: string }> {
    ...
    try {
      ...
      const existingUser = getDemoUserByEmail(credentials.email);
      if (existingUser) {
        throw new Error('User with this email already exists');
      }
  ...
  async login(credentials: LoginCredentials): Promise<{ user: AuthUser; token: string }> {
    ...
    try {
      ...
      const demoUser = getDemoUserByEmail(credentials.email);
      if (demoUser) {
  ...
  async updateUserSession(userId: string, socketId: string, isOnline: boolean = true): Promise<void> {
    try {
      if (isSupabaseConfigured) {
        await supabase
        ...
        updateDemoUserSession(userId, socketId, isOnline);
      }
    } catch (error) {
  ...
  async getOnlineUsersCount(): Promise<number> {
    try {
      if (isSupabaseConfigured) {
        ...
        return getDemoOnlineUsersCount();
      }
    } catch (error) {
  ...
  // Create initial portfolio for new user
  async createInitialPortfolio(userId: string): Promise<void> {
    try {
      if (isSupabaseConfigured) {
        await supabase
